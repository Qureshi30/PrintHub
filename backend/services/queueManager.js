...
class QueueManager {
  ...
  static async getCurrentQueue(limit = 50) {
    try {
      const queue = await Queue.find({
        ...
        .populate({
          path: 'printJobId',
          select: 'userId files totalCost estimatedPages status createdAt'
        });
  ...
  /**
   * Clean up orphaned queue items (where referenced PrintJob doesn't exist)
   * @returns {Promise<number>} Number of cleaned up items
   */
  static async cleanupOrphanedItems() {
    try {
      const queueItems = await Queue.find({}).select('printJobId');
      const printJobIds = queueItems.map(item => item.printJobId);
      
      const existingPrintJobs = await PrintJob.find({
        _id: { $in: printJobIds }
      }).select('_id');
      
      const existingIds = existingPrintJobs.map(job => job._id.toString());
      
      const orphanedItems = queueItems.filter(item => 
        !existingIds.includes(item.printJobId.toString())
      );
      
      if (orphanedItems.length > 0) {
        const orphanedIds = orphanedItems.map(item => item._id);
        await Queue.deleteMany({ _id: { $in: orphanedIds } });
        console.log(`🧹 Cleaned up ${orphanedItems.length} orphaned queue items`);
      }
      
      return orphanedItems.length;
    } catch (error) {
      console.error('❌ Error cleaning up orphaned items:', error.message);
      throw error;
    }
  }
}
